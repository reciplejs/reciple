...
export abstract class BaseManager<K extends string, V extends { id: string; }, R> {
    ...
    public resolve(resolvable: R|V|K): V|null {
        if (hasMixin(resolvable, this.holds)) return resolvable;
    ...
    public resolveId(resolvable: R|V|K): string|null {
        if ((typeof resolvable === 'object' && resolvable && 'id' in resolvable) || hasMixin(resolvable, this.holds)) return resolvable.id;
